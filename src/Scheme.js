...
class Scheme {
    ...
    constructor(schemeDescriptions) {
        if (!schemeDescriptions || typeof schemeDescriptions !== 'object') {
            throw new Error('Invalid schemeDescriptions argument. Should ba a plain object, describing a used schemes');
        }
        ...
        function _elementIterator(schemeName, data, elIndex) {
            ...
            for (const key in currentScheme) {
                if (hasOwnProperty(data, key)) {
                    ...
                    if (currentScheme[key].handlerAfter) handlers.push({func: currentScheme[key].handlerAfter, key});
                                ...
                                if (hasOwnProperty(currentScheme[key], 'valueScheme')) {
                                    if (typeof currentScheme[key].valueType === 'string') {
                                        if (currentScheme[key].valueType === 'array') {
                                            for (let index = 0; index < data[key][dataProperty].length; index++) {
                                                elementIterator(
                                            ...
                                            elementIterator(currentScheme[key].valueScheme, data[key][dataProperty]);
                                        }
                                    } else {
                                        this.appendError(
                                            currentScheme[key].valueScheme,
            ...
            for (const handler of handlers) {
                handler.func.call(this, 'validation', {resultElement: undefined, key: handler.key, data});
            }
    ...
    dataAdjust(dto, rootSchemeKey, options) {
        ...
        function _elementIterator(schemeKey, data, elIndex) {
            ...
            for (const key in currentScheme) {
                ...
                if (schemeAttribute.handlerAfter) handlers.push({func: schemeAttribute.handlerAfter, key});
                ...
                if (hasOwnProperty(currentScheme[key], 'scheme')) {
                    if (data[key] === null) {
                        ...
                        resultElement[key] = [];
                        for (let index = 0; index < data[key].length; index++) {
                            resultElement[key].push(elementIterator(currentScheme[key].scheme, data[key][index], index));
                        }
            ...
            for (const key in data) {
                if (!hasOwnProperty(currentScheme, key) && !localOptions.excludeUnnecessaryAttributes) {
    ...
    *dataAdjustGenerator(data, options) {
        ...
        function* _elementIterator(schemeName, data, elIndex) {
            ...
            for (const key in currentScheme) {
                ...
                // Если в данных нет атрибута, который есть в схеме
                if (!hasOwnProperty(data, key)) {
                    if (!localOptions.includeMissingAttributes && !currentScheme[key].required) continue;

                    // Если есть в схеме значение по умолчанию
                    if (hasOwnProperty(currentScheme[key], 'default')) {
                        if (isFn(currentScheme[key].default)) {
                            resultElement[key] = currentScheme[key].default.call(this, {resultElement, key, data});
                        } else {
                ...
                if (hasOwnProperty(currentScheme[key], 'scheme')) {
                    if (data[key] === null) {
                        ...
                        resultElement[key] = [];
                        for (let index = 0; index < data[key].length; index++) {
                            const element = yield* elementIterator(currentScheme[key].scheme, data[key][index], index);
    ...
    createEntity(dto, rootSchemeKey) {
        const datas = this.dataAdjust(dto, rootSchemeKey, {includeMissingAttributes: true, adjustTypes: true});
...
function validateValue(type, restrictions, value, options) {
    if (options.validationType) {
        ...
        if (!typeValid) return {valid: false, message: `Wrong type for value ${value}`, typeError: true};
    }
